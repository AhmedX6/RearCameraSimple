...
bool decode_yuv_neon(unsigned char *out, unsigned char const *y, unsigned char const *uv, int width, int height, unsigned char fill_alpha = 0xff)
{
    ...
    if (0 != (width & 1) || width < 2 || 0 != (height & 1) || height < 2 || !out || !y || !uv)
        return false;
    ALOGD("decode_yuv_neon params %d %d", width, height);
    // in & out pointers
    unsigned char *dst = out;

    // constants
    int const stride = width * trait::bytes_per_pixel;
    ALOGD("stride:%d", stride);
    int const itHeight = height >> 1;
    ALOGD("height:%d", itHeight);
    int const itWidth = width >> 3;
    ALOGD("width:%d", itWidth);
...
public:
    ...
    static PixelBlock init_pixelblock(unsigned char /*fill_alpha*/)
    {
        return uint8x8x3_t();
    }
    static uint8x8_t loadvu(unsigned char const *uv)
    {
        return vld1_u8(uv);
    }
